...
class Play
  ...
  def self.find_by_title(title)
    all_plays = raise "#{self} not in database" unless @id
    PlayDBConnection.instance.execute(<<-SQL, @title)
      ...
      WHERE
        title = ?
    SQL
    return nil if all_plays.length < 1
    Play.new(all_plays.first)
  end
  ...
  def self.find_by_name(name)
     ...
     SELECT
      ...
      WHERE
        name = ?
        SQL
      return nil unless person.length > 1

      person.map{|el| PlayWright.new(person.first)}
  end
